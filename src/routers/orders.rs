...
#[post("/crypto_order", format = "json", data = "<order_request>")]
pub async fn crypto_order_route(order_request: Json<OrderRequest>) -> Json<Response> {
    let mut engine = CryptoMatchingEngine::new();
    let pair = CryptoTradingPair::new("BTC".to_string(), "USD".to_string());
    engine.add_new_market(pair.clone());

    let order = Order::new(match order_request.order_type.as_str() {
        "buy" => BidOrAsk::Bid,
        "sell" => BidOrAsk::Ask,
        _ => return Json(Response { message: "Invalid order type".to_string() }),
    }, order_request.price);

    match engine.place_limit_order(pair, order_request.amount, order) {
...
#[post("/stock_order", format = "json", data = "<order_request>")]
pub async fn stock_order_route(order_request: Json<OrderRequest>) -> Json<Response> {
    let mut stock_engine = StockMatchingEngine::new();
    let symbol = StockTradingSymbol::new("AAPL".to_string());
    ...
    match stock_engine.place_limit_order(symbol, order_request.amount, order) {
